...
apt update -y
apt upgrade -y
apt install -y sudo kde-plasma-desktop sddm nano passwd
...
cat <<EOF > $HOME/startkde_debian.sh
#!/data/data/com.termux/files/usr/bin/bash
...
# and run KDE as user.
proot-distro login debian --shared-tmp -- /bin/bash -c  'export PULSE_SERVER=127.0.0.1 && export XDG_RUNTIME_DIR=\${TMPDIR} && su - $username -c "env DISPLAY=:0 startplasma-x11"'

exit 0
EOF

# Make the start script executable
chmod +x $HOME/startkde_debian.sh

echo "${G}User $username added successfully and configured.${W}"
echo "${C}To start KDE desktop, type:${W}"
echo "  ./startkde_debian.sh"
sleep 5
rm install.txt